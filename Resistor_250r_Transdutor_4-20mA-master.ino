...
apropriado às entradas analógicas do Arduino ou outro sistema microcontrolado). 
...
float ICal=1.000, VCal=1.000; /* Valores multiplicadores para calibração de leituras via firmware. Trabalhar dentro do range 0,
                               * 700-1,300 (valores acima ou abaixo desse range indicam transdutor ou aferidor/padrão de 
...
float Tens(byte pin) { // Função para Leitura de Tensão
  ...
  unsigned int samps = 200;   /* Variável que guarda a quantidade de amostras realizadas (amostragem). Oversampling para 
                               ...
                               * considerações devem ser feitas para evitar overflows.
                               */
